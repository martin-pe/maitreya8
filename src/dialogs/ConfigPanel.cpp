...
ConfigPanel *ConfigPanelFactory::createPanel( const int &id, wxWindow *parent )
{
	...
	case CONFIGPANEL_WESTERNASPECT:
		...
		break;
#ifdef USE_URANIAN_CHART
	case CONFIGPANEL_URANIAN:
		panel = createUranianPanel( parent );
		break;
	case CONFIGPANEL_URANIANCHART:
		panel = createUranianChartPanel( parent );
		break;
	case CONFIGPANEL_URANIANEVALUATION:
		panel = createUranianEvaluationPanel( parent );
		break;
#endif
	default:
		assert( false );
